...
class LeftPanelAroundUser extends Component {
  constructor(props) {
    ...
    this.computDitance = this.computDitance.bind(this);
    this.state = {
      lat1: false,
      lng1: false,
      list: [],
  ...
  componentDidMount() {
    ...
    store.on(typeOfActions.LOCATION_DETECTED, this.computDitance);
  }
  componentWillUnmount() {
    ...
    store.removeListener(typeOfActions.LOCATION_DETECTED, this.computDitance);
  }
  dataReceived() {
  ...
  computDitance() {
    if (this.state.dataReceived) {
      navigator.geolocation.getCurrentPosition( () => {
        ...
        let lat1 = store.location.latitude;
        let lng1 = store.location.longitude;
