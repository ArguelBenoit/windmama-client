...
class GraphWidget extends Component {
  ...
  canvasCreator() {
    ...
    items.forEach( (el,i) => {
      ...
      if (i === 0) {
        ctx1.lineTo(0, heightCanvas - (el.avg/1.852) * 3);
        ctx1.lineTo(widthPlot*0.5, heightCanvas - (el.avg/1.852) * 3);
        if (el.avg && el.avg > 0) {
          ctx1.font = '14px Helvetica';
          ctx1.fillStyle = getColor(el.avg);
          ctx1.fillText(
            windUnit(el.avg),
            widthPlot*(i+0.5),
        ...
        ctx1.lineTo(widthCanvas, heightCanvas - (el.avg/1.852) * 3);
    ...
    const grd = ctx1.createLinearGradient(0,heightCanvas,0,0);
    ...
    items.forEach( (el,i) => {
      ...
      if (i === 0) {
        ctx2.lineTo(0, heightCanvas - (el.max/1.852) * 3);
        ctx2.lineTo(widthPlot*0.5, heightCanvas - (el.max/1.852) * 3);
        if (el.max && el.max > 0) {
          ctx2.font = '14px Helvetica';
          ctx2.fillStyle = getColor(el.max);
          ctx2.fillText(
            windUnit(el.max),
            widthPlot*(i+0.5),
        ...
        ctx2.lineTo(widthCanvas, heightCanvas - (el.max/1.852) * 3);
